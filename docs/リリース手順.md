...
- [Termnix-IT/CIPHER-VAULT](https://github.com/Termnix-IT/CIPHER-VAULT)
...
アプリ内の手動更新には、同じビルドの `setup.exe` と `latest.yml` が必須です。生成された setup 用 `blockmap` も一緒に添付します。詳しくは [手動更新とアイコン](手動更新とアイコン.md) を参照してください。

v0.2.0 以降は配布ファイル名に空白を使用せず、`CIPHER-VAULT-...` に統一します。公開前に `latest.yml` の `files[].url` と添付する setup のファイル名が完全一致することを確認します。GitHub 側のファイル名変換に頼らず、ダウンロード先の不一致を防ぎます。

CLI で公開する場合も、まず draft を作成し、setup・portable・更新情報・blockmap のアップロードを確認してから公開します。タグはリリース用コミットに固定し、既存タグや公開済み資産を上書きしません。
...
初回デスクトップ公開は `v0.1.0` です。以後は既存の公開版より大きいバージョン番号を使用します。
...
- [v0.1.0 リリースノート](リリースノート/v0.1.0.md)
